...
const studentCommittee = [
  ...
  },
  {
    name: "Mr. Tejas",
    title:
      "JuristQuest –Student Coordinator – Karnataka, AP & Telangana \nAdvocate",
    ...
    for (let i = 0; i < members.length; i += 3) {
      ...
      if (isLastRow && rowItems.length < 3) {
        rows.push(
          <div
            key={i}
            className="hidden sm:flex justify-center gap-8 flex-wrap"
          >
        ...
        rows.push(
          <div
            key={i}
            className="hidden sm:grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-6xl mx-auto"
          >
            {rowItems.map((member, index) => (
              <div
                key={index}
    ...
    <section className="bg-gradient-to-b from-white to-[#f3f6ec] py-24 px-6 font-['Lato']">
      <div className="max-w-7xl mx-auto text-center mb-14">
        ...
        <h2 className="text-3xl md:text-4xl font-bold text-gray-800">
          ...
          <span className="text-[#6B21A8]">JuristQuest 2025</span>
        </h2>
        <p className="mt-4 text-gray-600 text-sm max-w-2xl mx-auto">
          A fusion of seasoned legal minds and visionary leaders committed to
          delivering an extraordinary moot court experience.
        </p>
      </div>
